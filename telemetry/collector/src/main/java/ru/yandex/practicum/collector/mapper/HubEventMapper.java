...
public class HubEventMapper {
    ...
    public HubEventAvro toAvro(HubEvent event) {
        ...
        switch (event.getType()) {
            ...
            case DEVICE_ADDED -> builder.setPayload(mapDeviceAdded((DeviceAddedEvent) event));

            case DEVICE_REMOVED -> builder.setPayload(mapDeviceRemoved((DeviceRemovedEvent) event));

            case SCENARIO_ADDED -> builder.setPayload(mapScenarioAdded((ScenarioAddedEvent) event));

            case SCENARIO_REMOVED -> builder.setPayload(mapScenarioRemoved((ScenarioRemovedEvent) event));
        }
    ...
    private ScenarioConditionAvro mapCondition(ScenarioCondition c) {
        return ScenarioConditionAvro.newBuilder()
                ...
                .setValue(validateScenarioValueType(c.getValue()))
                .build();
    }
    ...
    private Object validateScenarioValueType(Object value) {
        if (value instanceof Integer || value instanceof Boolean || value == null) {
            return value;
        }
        throw new IllegalArgumentException("Значение условия не поддерживается: " + value);
    }
