    ...
    private SpecificRecord mapPayload(SensorEvent event) {
        ...
        return switch (event.getType()) {
            ...
            case LIGHT_SENSOR_EVENT ->
                    mapLight((LightSensorEvent) event);

            case MOTION_SENSOR_EVENT ->
                    mapMotion((MotionSensorEvent) event);

            case SWITCH_SENSOR_EVENT ->
                    mapSwitch((SwitchSensorEvent) event);

            case TEMPERATURE_SENSOR_EVENT ->
                    mapTemperature((TemperatureSensorEvent) event);

            case CLIMATE_SENSOR_EVENT ->
                    mapClimate((ClimateSensorEvent) event);
        };
    }
